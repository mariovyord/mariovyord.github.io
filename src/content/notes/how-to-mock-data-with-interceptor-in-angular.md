...
export const mocksInterceptor: HttpInterceptorFn = (
  ...
  const existing = MOCK_ENDPOINTS.find((file) => req.url.endsWith(`/${file}`));

  if (!existing) {
    return next(req);
  }

  const fileName = getFileName(existing, req);

  // Redirect the request to a local folder
  const mockReq = req.clone({
    url: `/assets/mock-data/${fileName}.json`,
  });
...
function getFileName(fileName: string, req: HttpRequest<unknown>): string {
  // Additional checks that fit your needs...
